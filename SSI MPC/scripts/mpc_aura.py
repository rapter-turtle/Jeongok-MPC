...
class AuraMPC(Node):
    def __init__(self):
        ...
        self.alpha = np.zeros(self.M)   # RFF weights
    ...
    def run_SSI(self):
        ...
        # ---------- 1. 상태 및 입력 준비 ----------
        # z = [u, v, r, delta, F]
        z = np.array([self.u, self.v, self.r, self.delta, self.F])

        # ---------- 2. Feature phi = cos(w^T z + b) ----------
        phi = np.cos(self.w @ z + self.b)   # shape = (M,)

        # ---------- 3. h_hat(z) 계산 ----------
        h_hat = (self.alpha @ phi) / self.M   # scalar disturbance estimate

        # ---------- 4. residual 계산 ----------
        # residual = actual_dx - nominal_dx
        # nominal model derivative f(x,u) 필요 → acados model 사용 불가하므로 직접 3DOF 모델 넣어야 함
        # 일단 임시 더미 모델:
        M = 1.0  # Mass [kg]
        ...
        eps = 0.0001
        f_nom = np.array([
            ( - Xu*u - Xuu * sqrt(u * u + eps) * u + alpha1*T*cos(alpha2*delta)),
            self.u * np.sin(self.p),
            self.r
        ])
        ...
        # residual (scalar) — yaw rate 방정식 기준으로 먼저 시작
        residual = dx_actual[5] - f_nom[2]   # r_dot error only

        # ---------- 5. α 업데이트 ----------
        lr = 0.01   # learning rate
        self.alpha = self.alpha - lr * (h_hat - residual) * phi

        # ---------- 6. 저장 ----------
        self.states_prev = self.states.copy()

        # ---------- 7. disturbance publishing ----------
