...
def export_heron_model_nlp2() -> AcadosModel:
    ...
    p = vertcat(ox1, oy1, or1, bow_button)
    ...
    eps = 0.00001
    # dynamics
    T = 1.0*((1/(1+exp(s*F_eff)))*(b11*F_eff + tanh(k*F_eff)*a1) + (1/(1+exp(-s*F_eff)))*(b22*F_eff + tanh(k*F_eff)*a2))
    ...
    f_expl = vertcat(u*cos(psi) - v*sin(psi),
                     ...
                     ( - Nr*r - Nrr * sqrt(r * r + eps)*r - b3*T*sin(b2*delta) + F_l*F_bow*bow_button),
                     delta_d,
                     (F - F_eff)/tau,
                     F_d
                     )
...
def setup_trajectory_tracking_nlp2(x0, N_horizon, Tf, Q_mat, Q_mat_terminal, R_mat):
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-70.0,-0.5])
    ocp.constraints.ubu = np.array([+70.0,+0.5])
    ocp.constraints.idxbu = np.array([0, 1])

    ocp.constraints.lbx = np.array([-250.0, -0.5])
    ocp.constraints.ubx = np.array([250.0, 0.5])
