...
class AuraMPC(Node):
    def __init__(self):
        ...
        # Q_mat = 1*np.diag([1e1, 1e1, 1e3, 1e1, 1e1, 1e-2, 0, 1e0, 0])
        # Q_mat_terminal = 20*np.diag([1e2, 1e2, 1e3, 1e2, 1e2, 1e3, 0, 1e0, 0])
        # R_mat1 = 1*np.diag([1e-1, 1e-1, 5e1])
        # R_mat2 = 1*np.diag([1e-1, 1e-1])
        
        Q_mat = 1*np.diag([1e1, 1e1, 1e0, 1e1, 1e1, 1e-2, 0, 1e0, 0])
        Q_mat_terminal = 10*np.diag([1e2, 1e2, 1e3, 1e3, 1e3, 1e3, 0, 1e0, 0])
        R_mat1 = 1*np.diag([1e0, 1e0, 5e1])
        R_mat2 = 1*np.diag([1e0, 1e0])        
    ...
    def run(self):
        ...
        for j in range(self.N+1):
            ...
            yref1 = np.hstack((dock_x,dock_y,dock_psi,0,0,0,0,0,0,0,0,0))
            yref2 = np.hstack((dock_x,dock_y,dock_psi,0,0,0,0,0,0,0,0))
            if j == self.N:
                yref1 = np.hstack((dock_x,dock_y,dock_psi,0,0,0,0,0,0))
                yref2 = np.hstack((dock_x,dock_y,dock_psi,0,0,0,0,0,0))
        ...
        self.F_eff = self.ocp_solver_nlp2.get(1, "x")[7]
        self.bow_switch = CIA_bow_array[0]

        # self.get_logger().info(f"MPC Computation Time: {t_preparation + t_feedback:.4f}s")


        # if np.sqrt((self.states[0]-dock_x)**2 + (self.states[1]-dock_y)**2) < 1.5 and np.sqrt((self.states[2])**2 + (self.states[3])**2) < 1.0:
