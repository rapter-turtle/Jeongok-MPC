...
class AuraMPC(Node):
    def __init__(self):
        ...
        Q_mat_terminal1 = 1*np.diag([1e3, 1e3, 1e6, 1e3, 1e3, 1e-2, 1e-1, 1e0])
        R_mat1 = 1*np.diag([1e1, 1e-1, 1e4])

        Q_mat2 = 1*np.diag([1e1, 1e1, 1e3, 1e3, 1e3, 1e-2, 1e0])
        Q_mat_terminal2 = 1*np.diag([1e3, 1e3, 1e6, 1e4, 1e4, 1e-2, 1e0])
        ...
        # Q_mat1 = 1*np.diag([1e2, 1e2, 1e5, 1e3, 1e3, 1e-2, 1e-1, 1e0])
        # Q_mat_terminal1 = 1*np.diag([1e3, 1e3, 1e5, 1e3, 1e3, 1e-2, 1e-1, 1e0])
        # R_mat1 = 1*np.diag([1e1, 1e-1, 1e3])
    ...
    def run(self):
        ...
        if self.count_on_left <= dwell_len - 1 and self.gear_switch > 0.1:
            ...
            input_gear_array = bow_mapping(model, gear_array[start_idx:], self.gear_switch, self.con_dt, len(gear_array[start_idx:]),
            ...
            self.count_on_right = 0
            CIA_gear_array = bow_mapping(model, gear_array, self.gear_switch, self.con_dt, len(gear_array), dwell_time, stop_dwell_time)
        ...
        if self.bow_count_on_left <= bow_dwell_len - 1 and self.bow_switch > 0.1:
            ...
            self.bow_count_on_right = 0
            CIA_bow_array = bow_mapping(model, bow_array, self.bow_switch, self.con_dt, len(bow_array), bow_dwell_time, bow_stop_dwell_time)
        ...
        # print(self.bow_count_on_left, self.bow_count_on_right, self.bow_count_on_zero)
        # print(CIA_bow_array) 
        t1 = time.time()
        ...
        ##### Obstacle Position ######
        for j in range(self.N):
            obs_pos = np.array([dock_x, dock_y, dock_psi, CIA_gear_array[j], CIA_bow_array[j]]) 
        ...
        self.F = CIA_gear_array[0]
        self.gear_switch = CIA_gear_array[0]

        self.bow_thrust = CIA_bow_array[0]
        ...
        # actuator_msg.data = [self.delta_pwm, self.F_pwm, -1.0, 0.0]
