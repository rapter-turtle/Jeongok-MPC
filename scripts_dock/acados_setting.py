...
from casadi import SX, vertcat, sin, cos, sqrt, log, exp
...
def export_heron_model() -> AcadosModel:
    ...
    # xh = xn
    # yh = yn
    # h_dock = -log(ox2*(yh-oy1) ** 2 + 1) - (xh - ox1)
    # h_dock_dot = -2*ox2*(yh-oy1)*yh_dot/(ox2*(yh-oy1)**2 + 1) - xh_dot
    ...
    h_expr[0] = h_dock1_dot + 10.0*h_dock1
    h_expr[1] = h_dock2_dot + 10.0*h_dock2
    h_expr[2] = -(u*cos(psi) - v*sin(psi)) + oy2*(-xn + ox1)
...
def setup_trajectory_tracking(x0, N_horizon, Tf):
    ...
    Q_mat = 1*np.diag([0, 1e3, 1e1, 1e0, 1e0, 1e-2, 1e-1, 1e-2])
    Q_mat_terminal = np.diag([1e3, 1e3, 1e2, 1e-1, 1e-1, 1e-1, 1e-1, 1e-5])
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-100.0,-300])
    ocp.constraints.ubu = np.array([+100.0,+300])
    ocp.constraints.idxbu = np.array([0, 1])

    ocp.constraints.lbx = np.array([-100,-100,-250.0, -900])
    ocp.constraints.ubx = np.array([100,100,250.0, 1600.0])
    ocp.constraints.idxbx = np.array([0,1,6, 7])
