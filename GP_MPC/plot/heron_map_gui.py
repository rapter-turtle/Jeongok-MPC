...
class SensorFusionEKF(Node):
    def __init__(self):
        ...
        kaist_img = plt.imread("/home/user/aura_ws/src/aura_mpc/GP_MPC/plot/jeongok.png")
        ...
        self.refs, = self.ax1.plot(map_traj_data[:,0],map_traj_data[:,1],color='grey', linestyle='--',label='reference traj.')
        self.ref_line, = self.ax1.plot([], [], 'b-', label='Reference')        
        self.pred_line, = self.ax1.plot([], [], 'g-', label='Predicted')


        self.refs2, = self.ax1.plot(map_traj_data[:,0],map_traj_data[:,1], color='grey', linestyle='--')
        ...
        # self.ax1.set_ylim([360.0, 380.0])
        # self.ax1.set_xlim([470.0, 490.0])
        # self.ax1.legend()
        ...
        # self.line7_right, = self.ax7.plot([], [], 'g-', label='DOB_u')
        # self.ax7.set_xlabel('Time')
        # self.ax7.set_ylabel('DOB_u')
        ...
        self.arrow_length = 5.0/(x_actual_max - x_actual_min)*self.map_width
        size = 1.5
    ...
    def traj_callback(self, msg):
        ...
        if self.plot_traj != data :
            ...
            if self.plot_ref_dt > 0.0 and self.plot_ref_dt < 10.0:  
                    # Generate the trajectory using the updated parameters
                trajectory_data = generate_figure_eight_trajectory(
                    self.plot_ref_num,	
                    self.plot_ref_dt,
            ...
            elif self.plot_ref_dt < 0.0:
                trajectory_data = generate_figure_LARS_trajectory(
                    self.plot_ref_num,	
                    self.plot_ref_dt,
                    (self.plot_traj_x, self.plot_traj_y),	
                    self.plot_theta,
                ...
                self.obs_list[0:2] = self.utm_to_map(x_center + 400.0,y_center + 400.0)
                self.obs_list[2] = 0.1
                self.obs_list[3:5] = self.utm_to_map(x_center + 400.0,y_center + 400.0)
                self.obs_list[5] = 0.1            
            
            elif self.plot_ref_dt > 90.0 and self.plot_ref_dt < 120.0:
                trajectory_data, trajectory_data2 = generate_figure_dock(
                    self.plot_ref_num,	
                    self.plot_ref_dt,
                    (self.plot_traj_x, self.plot_traj_y),	
                    self.plot_theta,
                ...
                map_traj_data = np.empty_like(trajectory_data2)
                for i in range(trajectory_data2.shape[0]):
                    pos_x, pos_y = trajectory_data2[i, 0] + self.traj_offset[0], trajectory_data2[i, 1] + self.traj_offset[1]
                    pos_x_map, pos_y_map = self.utm_to_map(pos_x, pos_y)
                    map_traj_data[i, 0] = pos_x_map
                    map_traj_data[i, 1] = pos_y_map

                # Update the reference trajectory plot
                self.refs2.set_data(map_traj_data[:, 0], map_traj_data[:, 1])  # Update the main plot
    ...
    def mpc_callback(self, msg):# - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문        
        ...
        elif self.plot_ref_dt > 50.0 and self.plot_ref_dt < 120.0:
        ...
        elif self.plot_ref_dt > 130.0:
            trajectory_data = generate_figure_bow(
                self.plot_ref_num,	
                self.plot_ref_dt,
                (self.plot_traj_x, self.plot_traj_y),	
                self.plot_theta,
