...
def forward_prediction(state, control_input, delay, dt_control, disturbance, dt_sim=0.01):
    ...
    def dynamics(x, delta_cmd, F_cmd):
        ...
        # Dynamics
        u_dot = ( ( gp_surge_np(u, T, delta_cmd))) 
        v_dot = ( -Yv*v - Yr*r - Yvv * np.sqrt(v * v + eps) * v + alpha3*T*np.sin(alpha2*delta_cmd)) 
        r_dot = ( - Nr*r - Nv*v - Nrr * np.sqrt(r * r + eps) * r - alpha4*T*np.sin(alpha2*delta_cmd))
            

        # u_dot = ( - Xu*u - Xuu * np.sqrt(u * u + eps) * u + alpha1*T*np.cos(alpha2*delta_cmd))
        # u_dot = ( ( gp_surge_np(u, T, delta_cmd))) - disturbance[0]
        # v_dot = ( -Yv*v - Yr*r - Yvv * np.sqrt(v * v + eps) * v + alpha3*T*np.sin(alpha2*delta_cmd)) - disturbance[1]
        # r_dot = ( - Nr*r - Nv*v - Nrr * np.sqrt(r * r + eps) * r - alpha4*T*np.sin(alpha2*delta_cmd)) - disturbance[2]
