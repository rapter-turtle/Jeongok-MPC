...
class AuraMPC(Node):
    ...
    def run(self):
        ...
        if time.time() - self.start_time > 1:          
            ...
            # self.reference = generate_figure_eight_trajectory_con(1000, self.ref_dt, self.plot_traj_xy, self.theta, self.A, self.B, self.C) # reference dt = 0.01 sec, 1000 sec trajectory generation
            self.reference = generate_figure_eight_trajectory(1000, self.ref_dt, self.plot_traj_xy, self.theta, self.A, self.B, self.C, self.max_speed) # reference dt = 0.01 sec, 1000 sec trajectory generation
