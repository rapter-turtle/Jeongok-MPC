...
class ShipSimulator(Node):
    def __init__(self):
        ...
        self.gp_surge_model = build_gp_surge_model(GP_NPZ_PATH)
    ...
    def ship_dynamics(self, state, control, dt):
        ...
        u_dot = self.gp_surge_model(u, T, delta) - 0.2 
        v_dot = (-Yv*v - Yr*r - Yvv * np.sqrt(v* v + eps) * v + alpha3*T*np.sin(alpha2*delta))/M - 0.2
