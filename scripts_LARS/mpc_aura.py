...
class AuraMPC(Node):
    def __init__(self):
        ...
        self.B = 100.0
        self.C = 50.0
        self.theta = 20.0*np.pi/180
        self.plot_traj_xy = (ship_state_x, ship_state_y)
        
        self.ref_dt = -0.01
        self.ref_iter = int(self.con_dt/self.ref_dt)
        self.reference = generate_figure_LARS_trajectory(300, self.ref_dt, self.plot_traj_xy, self.theta, self.A, self.B, self.C)[::-1] # reference dt = 0.01 sec, 1000 sec trajectory generation
    ...
    def convert_thrust_to_pwm(self, rpm_thrust):
        ...
        if thrust < 0.0:
            ...
            return self.clamp(pwm, 1000.0, 1500.0)  # Any value <= 0 thrust maps to PWM 1000
        else:
            # Calculate PWM based on the thrust
            pwm = 3.9 * thrust + 1550.0
            # You can switch the formula if needed, using the commented one
            # pwm = 5.0 * thrust + 1500
            return self.clamp(pwm, 1500.0, 2000.0)  # Ensure PWM is within the bounds
    ...
    def run(self):
        ...
        obs_pos = np.array([200.0, +200.0, 1.0,  # Obstacle-1: x, y, radius
                            -300.0, -300.0, 1.0, # Obstacle-2: x, y, radius
        ...
        self.F_pwm = self.convert_thrust_to_pwm(self.F)                        
