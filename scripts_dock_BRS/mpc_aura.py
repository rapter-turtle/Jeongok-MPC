...
class AuraMPC(Node):
    def __init__(self):
        ...
        self.Tf = 10 # prediction time 4 sec
    ...
    def run(self):
        ...
        if time.time() - self.start_time > 10.0:                
            ...
            for j in range(self.N+1):
                ...
                yref = np.hstack((real_dock-2.0,dock_y,dock_psi,0.5,0,0,0,0,0,0,0))
                if j == self.N:
                    yref = np.hstack((dock_x,dock_y,dock_psi,0.5,0,0,0,0,0))
            ...
            # if np.sqrt((self.states[0]-dock_x+2)**2 + (self.states[1]-dock_y)**2)<=2.0 and np.sqrt((self.states[3])**2 + (self.states[4])**2) < 1.0:
            #     self.stop_switch = 1.0
            if self.states[0]-dock_x+2.0 > 0.0 and np.abs(self.states[1]-dock_y) < 2.0 and self.stop_switch == 0:
                self.stop_switch = 1.0
                self.fin_u = self.states[3]
                self.fin_v = self.states[4]
                self.fin_r = self.states[5]
                
            if self.stop_switch == 1.0:
                self.F = 0.0
                print("end")
                print(self.fin_u, self.fin_v, self.fin_r)
            else:
