...
def export_heron_model_nlp2() -> AcadosModel:
    ...
    p = vertcat(ox1, oy1, or1, thrust_button)
    ...
    f_expl = vertcat(u*cos(psi) - v*sin(psi),
                     ...
                     ( -Yv*v - Yvv * sqrt(v * v + eps) * v  - Yr*r + thrust*thrust_button*sin(b2*delta)),
                     ( - Nr*r - Nrr * sqrt(r * r + eps)*r - Nv*v - b3*thrust*thrust_button*sin(b2*delta)),
                     delta_d
                     )
...
def setup_trajectory_tracking_nlp2(x0, N_horizon, Tf, Q_mat, Q_mat_terminal, R_mat):
    ...
    ocp.constraints.lbx = np.array([-200.0])
    ocp.constraints.ubx = np.array([200.0])
