...
def export_heron_model_nlp1() -> AcadosModel:
    ...
    f_expl = vertcat(u*cos(psi) - v*sin(psi),
                     ...
                     ( -Yv*v - Yvv * sqrt(v * v + eps) * v - Yr*r + 2.2*2.2*F*sin(b2*delta)),
                     ( - Nr*r - Nrr * sqrt(r * r + eps)*r - Nv*v - b3*2.2*2.2*F*sin(b2*delta)),
                     delta_d,
                     F_d
                     )
...
def setup_trajectory_tracking_nlp1(x0, N_horizon, Tf, Q_mat, Q_mat_terminal, R_mat):
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-50.0,-1])
    ocp.constraints.ubu = np.array([+50.0,+1])
    ocp.constraints.idxbu = np.array([0, 1])

    ocp.constraints.lbx = np.array([-200.0, -1.0])
    ocp.constraints.ubx = np.array([200.0, 1.0])
